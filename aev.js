...
function calculate() {
	...
	if (document.getElementById('p75').checked==true) {p38='ING'}
		else{p38='  '}
...
function exportarPDF(){
	...
	if(answerCounter.length >= 1){
		...
		doc.addImage(imgData, 'JPEG', 15, 40,148,210);



		//Agrego texto con variables
		doc.text(`Digestivo: ${D}`, 10, 10);
		doc.text(`Intestinal: ${I}`, 10, 20);
		doc.text(`Circulatorio: ${C}`, 10, 30);
		doc.text(`Nervioso: ${N}`, 10, 40);
		doc.text(`Inmunológico: ${M}`, 10, 50);
		doc.text(`Respiratorio: ${R}`, 10, 60);
		doc.text(`Urinario: ${U}`, 10, 70);
		doc.text(`Glandular: ${G}`, 10, 80);
		doc.text(`Estructural: ${E}`, 10, 90);
